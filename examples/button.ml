let main () =
  ...
  let button = Objects.ButtonG.button ~packing:(hbox#pack ~padding:5) () in
  let _ = button#connect#clicked ~callback:
      (fun () -> prerr_endline "Hello again - cool button was pressed") in
  let button = Objects.ButtonG.button ~use_underline:true ~label:"_Coucou" ~packing:(hbox#pack ~padding:5) () in
  let _ = button#connect#clicked ~callback:
