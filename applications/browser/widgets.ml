...
class multibox ~rows ~columns ?(row_view = rows) ?(col_view = columns)
  ...
  let sw =
    ScrolledWindowG.scrolled_window ~hscrollbar_policy:`AUTOMATIC ~vscrollbar_policy:`AUTOMATIC
      ?show ?packing () in
  let vp = ViewportG.viewport ~shadow_type:`NONE ~packing:sw#add in
  let table =
    TableG.table ~n_columns:columns ~n_rows:rows ~homogeneous:true ~packing:vp#add () in
  let buttons =
    Array.init columns
      (fun left -> Array.init rows
	(fun top -> ButtonG.button
              ~packing:(Lablgtk3Compat.attach table ~top ~left ~expand:`BOTH)))
  in
  object (self)
