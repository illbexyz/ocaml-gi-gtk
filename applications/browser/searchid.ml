...
let rec search_type_in_signature t ~sign ~prefix ~mode =
  ...
  begin fun item -> match item with
	  ...
	  | Type_open -> false
          | Type_variant l ->
            List.exists l ~f:
            begin fun {cd_args=a; cd_res=r; _} ->
              matches_args a ||
      ...
      | Sig_module (id, _, {md_type=Mty_signature sign;_}, _, _) ->
...
let search_all_types t ~mode =
  ...
  in List2.flat_map !module_list ~f:
    ...
    with {md_type=Mty_signature sign;_} ->
        List2.flat_map tl
  ...
  List.iter (search_module str ~prefix) ~f:
    begin fun item ->
      ...
      | Pstr_typext l when kind = Ptype ->
          ...
          false
      | Pstr_exception pcd when kind = Pconstructor -> name = pcd.ptyexn_constructor.pext_name.txt
  ...
  List.iter (search_module_type sign ~prefix) ~f:
    begin fun item ->
      ...
      | Psig_typext l when kind = Pconstructor ->
          ...
          false
      | Psig_exception pcd when kind = Pconstructor -> name = pcd.ptyexn_constructor.pext_name.txt
