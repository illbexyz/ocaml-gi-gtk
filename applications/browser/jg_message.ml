...
let formatted ~title ?on ?(ppf = Format.std_formatter)
  ...
  let frame =
    match on with
      Some frame ->
        (frame :> ContainerG.container)
    | None ->
        let tl = WindowG.window ~title () in
        (HBoxG.h_box ~packing:tl#add () :> ContainerG.container)
  in
  let sw =
    ScrolledWindowG.scrolled_window ~hscrollbar_policy:`AUTOMATIC ~vscrollbar_policy:`AUTOMATIC
      ~packing:frame#add () in
  let tw = TextViewG.text_view ~packing:sw#add ()  in
...
let ask ~title ?master ?(no=true) ?(cancel=true) text =
  let tl = DialogG.dialog ~title ~modal:true () in
  Gaux.may (fun w -> tl#set_transient_for w#as_window) master;
  LabelG.label ~label:text ~packing:tl#vbox#add ~xpad:20 ~ypad:10
