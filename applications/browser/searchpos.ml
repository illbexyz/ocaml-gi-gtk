...
let rec search_pos_signature l ~pos ~env =
  ...
  begin fun env pt ->
    ...
    if in_loc ~pos pt.psig_loc then
      ...
      | Psig_modtype {pmtd_type=Some t;_} ->
...
let rec view_signature ?title ?path ?(env = !start_env) ?(detach=false) sign =
  ...
  let tw, finish =
    try match path, !default_frame with
      None, Some ({mw_title=Some label;_} as mw) when not detach ->
        List.iter (mw.mw_frame#children @ mw.mw_buttons#children)
          ~f:(fun w -> w#destroy ());
        let detach =
          ButtonG.button ~label:"Detach" ~packing:mw.mw_buttons#add () in
        ...
        List.iter2 ["Impl"; "Intf"] [".ml"; ".mli"] ~f:
          begin fun label ext ->
            try
              ...
              let button =
                ButtonG.button ~label ~packing:mw.mw_buttons#add () in
...
and view_decl_menu lid ~kind ~env =
  ...
  in
  let menu = new Lablgtk3Compat.factory (MenuG.menu ()) in
...
let rec search_pos_structure ~pos str =
  ...
  | Tstr_value (rec_flag, l) ->
      List.iter l ~f:
      begin fun {vb_pat=pat;vb_expr=exp;_} ->
        let env =
  ...
  | Tstr_include {incl_mod=m;_} -> search_pos_module_expr m ~pos
  | Tstr_primitive _
  | Tstr_type _
  | Tstr_typext _
  | Tstr_exception _
...
and search_pos_expr ~pos exp =
  ...
  | Texp_record {fields=l; extended_expression=opt; _} ->
      Array.iter l ~f:
  ...
  | Texp_letmodule (id, _, modexp, exp, _) ->
