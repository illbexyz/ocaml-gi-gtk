...
import           GtkDoc                         ( CRef )
import           Data.GI.CodeGen.ProjectInfo    ( authors
                                                , license
                                                , maintainers
                                                )
...
recurseWithState cgsSet cg = do
  ...
  case runCodeGen cg cfg (cgsSet cgs, info) of
    Left e -> throwError e
    Right (r, new) ->
      put (cgs, mergeInfoState oldInfo new) >> return (r, moduleCode new)
...
recurseWithAPIs apis cg = do
  ...
  -- Start the subgenerator with no code and no submodules.
  let info = cleanInfo oldInfo
      cfg' = cfg { loadedAPIs = apis, c2hMap = cToHaskellMap (M.toList apis) }
...
submodule (ModulePath (m : ms)) cg =
  submodule' m (submodule (ModulePath ms) cg)
...
-- rank type.
unwrapCodeGen
  :: CodeGen a -> CodeGenConfig -> (CGState, ModuleInfo) -> (a, ModuleInfo)
unwrapCodeGen cg cfg info = case runCodeGen cg cfg info of
...
evalCodeGen
  :: Config -> M.Map Name API -> ModulePath -> CodeGen a -> (a, ModuleInfo)
evalCodeGen cfg apis mPath cg =
  let initialInfo = emptyModule mPath
      cfg'        = CodeGenConfig { hConfig    = cfg
                                  , loadedAPIs = apis
...
transitiveModuleDeps minfo = Set.unions
  (moduleDeps minfo : map transitiveModuleDeps (M.elems $ submodules minfo))
...
qualified mp (Name ns s) = do
  ...
  when (modName cfg /= ns) $ registerNSDependency ns
  (_, minfo) <- get
...
qualifiedImport mp = do
  modify' $ \(cgs, s) ->
    (cgs, s { qualifiedImports = Set.insert mp (qualifiedImports s) })
...
describeCGError (CGErrorBadIntrospectionInfo e) =
  "Bad introspection data: " <> tshow e
...
-- dotWithPrefix mp = dotModulePath ("Bindings" <> mp)
