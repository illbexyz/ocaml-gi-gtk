...
import           Data.GI.GIR.Allocation         ( AllocationInfo(..)
                                                , AllocationOp(..)
                                                , unknownAllocationInfo
                                                )
...
import           Data.GI.GIR.Callback           ( Callback(..)
                                                , parseCallback
                                                )
import           Data.GI.GIR.Constant           ( Constant(..)
                                                , parseConstant
                                                )
import           Data.GI.GIR.Deprecation        ( DeprecationInfo )
import           Data.GI.GIR.Enum               ( Enumeration(..)
                                                , EnumerationMember(..)
                                                , parseEnum
                                                )
import           Data.GI.GIR.Field              ( Field(..) )
import           Data.GI.GIR.Flags              ( Flags(..)
                                                , parseFlags
                                                )
import           Data.GI.GIR.Function           ( Function(..)
                                                , parseFunction
                                                )
import           Data.GI.GIR.Interface          ( Interface(..)
                                                , parseInterface
                                                )
import           Data.GI.GIR.Method             ( Method(..)
                                                , MethodType(..)
                                                )
import           Data.GI.GIR.Object             ( Object(..)
                                                , parseObject
                                                )
import           Data.GI.GIR.Parser             ( Parser
                                                , runParser
                                                )
import           Data.GI.GIR.Property           ( Property(..)
                                                , PropertyFlag(..)
                                                )
import           Data.GI.GIR.Repository         ( readGiRepository )
import           Data.GI.GIR.Signal             ( Signal(..) )
import           Data.GI.GIR.Struct             ( Struct(..)
                                                , parseStruct
                                                )
import           Data.GI.GIR.Union              ( Union(..)
                                                , parseUnion
                                                )
import           Data.GI.GIR.XMLUtils           ( subelements
...
import           Data.GI.Base.BasicTypes        ( GType(..)
                                                , CGType
                                                , gtypeName
                                                )
...
parseNSElement aliases ns@GIRNamespace {..} element
  ...
  | otherwise = case nameLocalName (elementName element) of
...
parseNamespace element aliases = do
  ...
  let ns = GIRNamespace { nsName    = name
                        , nsVersion = version
                        , nsAPIs    = []
                        , nsCTypes  = []
                        }
...
parseRootElement aliases info@GIRInfoParse {..} element =
  case nameLocalName (elementName element) of
    ...
    "namespace" -> info
      { girIPNamespaces = parseNamespace element aliases : girIPNamespaces
      }
    _ -> info

emptyGIRInfoParse :: GIRInfoParse
emptyGIRInfoParse =
  GIRInfoParse { girIPPackage = [], girIPIncludes = [], girIPNamespaces = [] }

parseGIRDocument :: M.Map Alias Type -> Document -> GIRInfoParse
parseGIRDocument aliases doc = L.foldl' (parseRootElement aliases)
                                        emptyGIRInfoParse
                                        (subelements (documentRoot doc))
...
loadGIRFile
  ...
  -> [GIRRule]        -- ^ fixups
  -> IO (Document, M.Map (Text, Text) Document) -- ^ (loaded doc, dependencies)
loadGIRFile verbose name version extraPaths rules = do
  doc <-
    fixupGIRDocument rules <$> readGiRepository verbose name version extraPaths
  deps <- loadDependencies verbose
                           (documentListIncludes doc)
                           M.empty
                           extraPaths
                           rules
  return (doc, deps)
...
toGIRInfo info = case catMaybes (girIPNamespaces info) of
  [ns] -> Right GIRInfo
...
loadRawGIRInfo verbose name version extraPaths = do
  ...
  case toGIRInfo (parseGIRDocument M.empty doc) of
    Left err ->
      error . T.unpack $ "Error when raw parsing \"" <> name <> "\": " <> err
...
loadGIRInfo verbose name version extraPaths rules = do
  ...
  case combineErrors parsedDoc parsedDeps of
    Left err ->
      error . T.unpack $ "Error when parsing \"" <> name <> "\": " <> err
    Right (docGIR, depsGIR) -> do
      if girNSName docGIR == name
        then do
          forM_ (docGIR : depsGIR)
            $ \info -> girRequire (girNSName info) (girNSVersion info)
    ...
    Just ti -> do
      ...
      forM prereqGTypes $ \p -> do
        case M.lookup p csymbolMap of
          ...
          Nothing ->
            error
              $  "Could not find prerequisite type "
              ++ show p
              ++ " for interface "
              ++ show n
...
fixupField offsetMap f = f
  { fieldOffset = case M.lookup (fieldName f) offsetMap of
                    Nothing ->
                      error $ "Could not find field " ++ show (fieldName f)
...
fixupGIRInfos doc deps =
  (fixup fixupInterface >=> fixup fixupStruct >=> fixup fixupUnion) (doc, deps)
 where
  fixup
    :: (M.Map Text Name -> (Name, API) -> IO (Name, API))
    -> (GIRInfo, [GIRInfo])
    -> IO (GIRInfo, [GIRInfo])
...
fixupGIR rules elem = elem
  { XML.elementNodes = mapMaybe (\e -> foldM applyGIRRule e rules)
                                (XML.elementNodes elem)
  }
 where
  applyGIRRule :: XML.Node -> GIRRule -> Maybe XML.Node
  applyGIRRule n (GIRSetAttr (path, attr) newVal) =
    Just $ girSetAttr (path, attr) newVal n
  applyGIRRule n (GIRAddNode path new) = Just $ girAddNode path new n
  applyGIRRule n (GIRDeleteNode path ) = girDeleteNodes path n
...
girSetAttr (spec : rest, attr) newVal n@(XML.NodeElement elem) =
  if specMatch spec n
    then case rest of
      ...
      [] -> XML.NodeElement
        (elem
          { XML.elementAttributes = M.insert attr
                                             newVal
      ...
      _ -> XML.NodeElement
        (elem
          { XML.elementNodes = map (girSetAttr (rest, attr) newVal)
                                   (XML.elementNodes elem)
          }
        )
    else n
    ...
    -- Matched the full path, add the new child node.
      [] ->
        ...
        in  if nameLocalName newNode `elem` nodeNames
              then n
              else XML.NodeElement
                (element
                  { XML.elementNodes = XML.elementNodes element
      ...
      _ -> XML.NodeElement
        (element
          { XML.elementNodes = map (girAddNode rest newNode)
...
girDeleteNodes (spec : rest) n@(XML.NodeElement elem) = if specMatch spec n
  then case rest of
                                                               -- Matched the full path, discard the node
    [] -> Nothing
    -- More selectors to apply
    _  -> Just $ XML.NodeElement
      (elem
        { XML.elementNodes = mapMaybe (girDeleteNodes rest)
                                      (XML.elementNodes elem)
        }
      )
  else Just n
...
lookupAndMatch tag attrs attr = case M.lookup attr attrs of
  Just s -> case tag of
...
specMatch (GIRTypedName t name) (XML.NodeElement elem) =
  XML.nameLocalName (XML.elementName elem) == t && lookupAndMatch
    name
    (XML.elementAttributes elem)
    (xmlLocalName "name")
specMatch _ _ = False
