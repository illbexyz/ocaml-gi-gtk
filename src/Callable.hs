...
hOutType callable outArgs = do
  hReturnType <- case returnType callable of
    ...
    Just r ->
      if skipRetVal callable then return $ con0 "unit" else haskellType r
  hOutArgTypes <- forM outArgs $ \outarg -> wrapMaybe outarg >>= bool
    (haskellType (argType outarg))
    (maybeT <$> haskellType (argType outarg))
  nullableReturnType <- maybe (return False)
                              typeIsNullable
                              (returnType callable)
  let maybeHReturnType =
...
wrapMaybe arg =
  if mayBeNull arg then typeIsNullable (argType arg) else return False
...
inArgInterfaces args expose = do
 ...
 where
  ...
  go (arg : args) = do
    ...
    t' <- wrapMaybe arg >>= bool (return t) (return $ "Maybe (" <> t <> ")")
...
readInArrayLengths name callable hInArgs = do
  ...
  forM_ lengthMaps $ \(array, length, duplicate) ->
    when (array `elem` hInArgs) $ case duplicate of
...
prepareArgForCall omitted arg expose = do
  callback <- findAPI (argType arg) >>= \case
    Just (APICallback c) -> return (Just c)
    _                    -> return Nothing
  ...
  wrapMaybe arg >>= bool
    ...
    (do
      ...
      indent $ do
        ...
        line $ "Just " <> jName <> " -> do"
        indent $ do
          converted <- convert jName $ hToF (argType arg) (transfer arg)
...
prepareInCallback arg (Callback { cbCallable = cb }) expose = do
  ...
  (maker, wrapper, drop) <- case argType arg of
    ...
    _ ->
      terror $ "prepareInCallback : Not an interface! " <> T.pack (ppShow arg)

  wrapMaybe arg >>= bool
    (do
      let name'   = prime name
          dropped = case drop of
      ...
      line $ name' <> " <- " <> maker <> " " <> parenthesize
        (wrapper <> " " <> p <> " " <> dropped)
      when (scope == ScopeTypeAsync) $ line $ "poke " <> ptrName <> " " <> name'
      return name'
    )
    (do
      let maybeName = "maybe" <> ucFirst name
      ...
      indent $ do
        ...
        indent $ do
          let dropped = case drop of
                Just dropper -> parenthesize (dropper <> " " <> jName)
                Nothing      -> jName
          p <- if (scope == ScopeTypeAsync)
            then do
              ft <- typeShow <$> foreignType (argType arg)
...
prepareInoutArg arg = do
  ...
  case allocInfo of
    Just (TypeAllocInfo isBoxed n) -> do
      let allocator = if isBoxed then "callocBoxedBytes" else "callocBytes"
      wrapMaybe arg >>= bool
        (do
          name'' <-
            genConversion (prime name')
            $  literal
            $  M
            $  allocator
            <> " "
...
prepareOutArg arg = do
  ...
  if argCallerAllocates arg
    ...
    else genConversion name $ literal $ M $ "allocMem :: " <> typeShow
      (io $ ptr ft)
...
convertOutCArray
  ...
  -> (Text -> Text)
  -> ExcCodeGen Text
convertOutCArray callable t@(TCArray False fixed length _) aname nameMap transfer primeLength
  = do
    if fixed > -1
      then do
        ...
        lname' <- case Map.lookup lname nameMap of
          Just n  -> return n
          Nothing -> badIntroError $ "Couldn't find out array length " <> lname
...
formatHSignature callable symbol expose = do
  ...
  indent $ do
    ...
    let resultPrefix = if null (signatureArgTypes sig) then "" else "-> "
...
callableSignature
  :: Callable -> ForeignSymbol -> ExposeClosures -> ExcCodeGen Signature
callableSignature callable symbol expose = do
  let (hInArgs, _) = callableHInArgs
        callable
        (case symbol of
  ...
  return $ Signature
    ...
    , signatureArgTypes    = case symbol of
                               KnownForeignSymbol _ ->
                                 zip (map Just hInArgs) types
                               DynamicForeignSymbol w -> zip
                                 (Nothing : map Just hInArgs)
...
convertResult :: Name -> Callable -> Map.Map Text Text -> ExcCodeGen Text
convertResult n callable nameMap =
 ...
 where
  unwrappedConvertResult rname = case returnType callable of
        ...
        -- into Haskell values.
    Just (t@(TCArray False (-1) (-1) _)) -> badIntroError
      (  "`"
      <> tshow t
    ...
    Just (t@(TCArray False _ _ _)) ->
      convertOutCArray callable t rname nameMap (returnTransfer callable) prime
    Just t -> do
...
convertOutArg callable nameMap arg = do
  ...
  case argType arg of
    t@(TCArray False (-1) (-1) _) -> if argCallerAllocates arg
      then return inName
      else badIntroError
        (  "`"
        <> tshow t
    ...
    t@(TCArray False _ _ _) -> do
      ...
      let arrayLength = if argCallerAllocates arg then id else prime
          wrapArray a =
            convertOutCArray callable t a nameMap (transfer arg) arrayLength
      wrapMaybe arg >>= bool
        (wrapArray aname')
        (do
          line
...
genHaskellWrapper
  :: Name -> ForeignSymbol -> Callable -> ExposeClosures -> ExcCodeGen Text
...
genWrapperBody n symbol callable hInArgs hOutArgs omitted expose = do
  ...
  inArgNames <- forM (args callable)
    $ \arg -> prepareArgForCall omitted arg expose
  -- Map from argument names to names passed to the C function
  let nameMap =
        Map.fromList $ flip zip inArgNames $ map escapedArgName $ args callable
...
genDynamicCallableWrapper n typeSynonym callable = do
  ...
  let dyn =
        DynamicWrapper { dynamicWrapper = wrapper, dynamicType = typeSynonym }
