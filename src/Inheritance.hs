...
fullObjectInheritableList
  :: Inheritable i => Name -> Object -> CodeGen [(Name, i)]
...
fullInterfaceInheritableList
  :: Inheritable i => Name -> Interface -> CodeGen [(Name, i)]
...
removeDuplicates verbose inheritables =
 ...
 where
  filterDups
    :: M.Map Text (Bool, Name, i)
    -> (Name, i)
    -> CodeGen (M.Map Text (Bool, Name, i))
  filterDups m (name, prop) = case M.lookup (iName prop) m of
    Just (tainted, n, p)
      | tainted -> return m
      | (p == prop) -> return m
      | -- Duplicated, but isomorphic property
        otherwise -> do
        when verbose $ do
  ...
  filterTainted xs = [ (name, prop) | (_, (_, name, prop)) <- xs ]
...
fullObjectPropertyList n o =
  fullObjectInheritableList n o >>= removeDuplicates True
