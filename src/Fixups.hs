...
dropMovedItems (APIFunction f) =
  if fnMovedTo f == Nothing then Just (APIFunction f) else Nothing
...
guessObjectPropertyNullability obj = obj
  { objProperties = map (guessNullability (objMethods obj)) (objProperties obj)
  }

-- | Guess nullability for the properties of an interface.
guessInterfacePropertyNullability :: Interface -> Interface
guessInterfacePropertyNullability iface = iface
  { ifProperties = map (guessNullability (ifMethods iface)) (ifProperties iface)
  }
...
guessNullability methods =
  guessReadNullability methods . guessWriteNullability methods
    ...
    in  case findMethod methods ("set_" <> prop_name) of
          ...
          Just m ->
            ...
            in  if length (args c)
                     ...
                     && returnType c
                     == Nothing
                     && (transfer . last . args) c
                     == TransferNothing
                     && (direction . last . args) c
                     == DirectionIn
                     && methodMovedTo m
                     == Nothing
                     && methodType m
...
detectGObject (n, APIInterface iface) =
  if not (null (ifProperties iface) && null (ifSignals iface))
    then
      let gobject = Name "GObject" "Object"
      in  if gobject `elem` (ifPrerequisites iface)
            then (n, APIInterface iface)
            else
              ( n
              , APIInterface
                (iface { ifPrerequisites = gobject : ifPrerequisites iface })
              )
...
dropDuplicatedEnumFields enum = enum
  { enumMembers = dropDuplicates S.empty (enumMembers enum)
  }
 where
  dropDuplicates :: S.Set T.Text -> [EnumerationMember] -> [EnumerationMember]
  dropDuplicates _        []       = []
  dropDuplicates previous (m : ms) = if enumMemberName m `S.member` previous
...
checkClosureDestructors (n, APIStruct s) =
  ( n
  , APIStruct (s { structMethods = checkMethodDestructors (structMethods s) })
  )
...
checkMethodDestructors = map checkMethod
 where
  ...
  checkMethod m =
    m { methodCallable = checkCallableDestructors (methodCallable m) }
