...
mapNth :: Int -> (a -> a) -> [a] -> [a]
mapNth _ _ [] = []
mapNth n fn (x : xs) | n == 0    = fn x : xs
                     | otherwise = x : mapNth (n - 1) fn xs

-- | Delete recursively the file inside src
removeDirectoryContents :: FilePath -> IO ()
removeDirectoryContents src = do
  content <- getDirectoryContents src
  let xs = filter (`notElem` [".", ".."]) content
  forM_ xs $ \name -> do
    let srcPath = src </> name
    isDirectory <- doesDirectoryExist srcPath
    if isDirectory then removeDirectoryContents srcPath else removeFile srcPath

-- From https://stackoverflow.com/questions/6807025/what-is-the-haskell-way-to-copy-a-directory/6807279
copyDirectory :: FilePath -> FilePath -> IO ()
copyDirectory src dst = do
  whenM (not <$> doesDirectoryExist src)
    $ throw (userError "source does not exist")
  whenM (not <$> doesDirectoryExist dst) $ createDirectory dst
  content <- getDirectoryContents src
  let xs = filter (`notElem` [".", ".."]) content
  forM_ xs $ \name -> do
    let srcPath = src </> name
