...
import           Data.GI.GIR.XMLUtils           ( xmlLocalName
                                                , xmlNSName
                                                , GIRXMLNamespace
                                                    ( CGIRNS
                                                    , GLibGIRNS
...
defaultOverrides = Overrides { ignoredElems    = M.empty
                             , ignoredAPIs     = S.empty
...
emptyParserState = ParserState { currentNS = Nothing, flags = [] }
...
parseOverrides overrides = do
    runExceptT $ flip evalStateT emptyParserState $ execWriterT $ mapM
        (parseOneLine . T.strip)
        (T.lines overrides)
...
parseOneLine (T.stripPrefix "set-attr " -> Just s) = withFlags $ parseSetAttr s
parseOneLine (T.stripPrefix "add-node " -> Just s) = withFlags $ parseAdd s
...
parseIgnore (T.words -> [T.splitOn "." -> [api,elem]]) (Just ns) =
    tell $ defaultOverrides
        { ignoredElems = M.singleton (Name ns api) (S.singleton elem)
        }
parseIgnore (T.words -> [T.splitOn "." -> [api]]) (Just ns) =
    tell $ defaultOverrides { ignoredAPIs = S.singleton (Name ns api) }
parseIgnore ignore _ = throwError
...
parseSeal (T.words -> [s]) (Just ns) =
    tell $ defaultOverrides { sealedStructs = S.singleton (Name ns s) }
parseSeal seal _ = throwError
...
parseKeyValuePair p = case T.splitOn "=" p of
    [k, v] -> return (k, v)
    _ -> throwError ("Could not parse \"" <> p <> "\"as a \"key=value\" pair.")

-- | Mapping from GObject Introspection namespaces to pkg-config.
parsePkgConfigName :: Text -> Parser ()
parsePkgConfigName (T.words -> [gi,pc]) =
    tell $ defaultOverrides { pkgConfigMap = M.singleton (T.toLower gi) pc }
parsePkgConfigName t = throwError
...
parseNsVersion (T.words -> [ns,version]) =
    tell $ defaultOverrides { nsChooseVersion = M.singleton ns version }
parseNsVersion t = throwError
    (  "namespace-version syntax is of the form\n"
    <> "\t\"namespace-version namespace version\"\n"
    <> "Got \"namespace-version "
    <> t
    <> "\" instead."
    )
...
parseCabalPkgVersion (T.words -> [version]) =
    tell $ defaultOverrides { cabalPkgVersion = Just version }
parseCabalPkgVersion t = throwError
    (  "cabal-pkg-version syntax is of the form\n"
    <> "\t\"cabal-pkg-version version\"\n"
    <> "Got \"cabal-pkg-version "
    <> t
    <> "\" instead."
    )
...
parseSetAttr (T.words -> [path, attr, newVal]) = do
    ...
    tell $ defaultOverrides
        { girFixups = [GIRSetAttr (pathSpec, parsedAttr) newVal]
        }
parseSetAttr t = throwError
...
parseNodeSpec spec = case T.splitOn "@" spec of
    ...
    _       -> throwError ("Could not understand node spec \"" <> spec <> "\".")
...
parseXMLName a = case T.splitOn ":" a of
    ...
    _           -> throwError ("Could not understand xml name \"" <> a <> "\".")
...
parseVersion v = (chooseFullParse . readP_to_S V.parseVersion . T.unpack) v
  where
    chooseFullParse :: [(V.Version, String)] -> Parser V.Version
    chooseFullParse [] =
        throwError ("Could not parse version \"" <> v <> "\".")
...
checkPkgConfigVersion pkg op tVersion = do
    ...
    pcVersion <- liftIO (tryPkgConfig pkg) >>= \case
        Nothing ->
            throwError
                ("Could not determine pkg-config version for \"" <> pkg <> "\"."
                )
...
parseIf cond = case T.words cond of
    ...
    ("pkg-config-version" : rest) -> case rest of
        [pkg, op, version] -> checkPkgConfigVersion pkg op version >>= setFlag
        _                  -> throwError
...
parseEndif rest = case T.words rest of
    [] -> unsetFlag
    _  -> throwError ("Unexpected argument to 'endif': \"" <> rest <> "\".")
  where
    unsetFlag :: Parser ()
    unsetFlag = do
        s <- get
...
parseInclude fname = do
    ...
    liftIO (parseOverrides includeText) >>= \case
        Left err -> throwError
            ("Error when parsing included '" <> fname <> "': " <> err)
        Right ovs -> tell ovs
...
filterAllocInfo old new = AllocationInfo
    { allocCalloc = replace (allocCalloc old) (allocCalloc new)
...
filterOneAPI ovs (n, APIStruct s, maybeIgnores) =
    ( n
    , APIStruct s
        { structMethods        = maybe (structMethods s)
                                       (filterMethods (structMethods s))
                                       maybeIgnores
        , structFields         = if n `S.member` sealedStructs ovs
                                     then []
                                     else structFields s
        , structAllocationInfo = let ai = structAllocationInfo s
...
filterOneAPI ovs (n, APIUnion u, maybeIgnores) =
    ( n
    , APIUnion u
        { unionMethods        = maybe (unionMethods u)
                                      (filterMethods (unionMethods u))
                                      maybeIgnores
        , unionAllocationInfo = let ai = unionAllocationInfo u
                                in  case M.lookup n (allocInfo ovs) of
                                        Just info -> filterAllocInfo ai info
                                        Nothing   -> ai
        }
...
filterOneAPI _ (n, APIObject o, Just ignores) =
    ( n
    , APIObject o
        { objMethods = filterMethods (objMethods o) ignores
        , objSignals = filter ((`S.notMember` ignores) . sigName) (objSignals o)
        }
    )
filterOneAPI ovs (n, APIInterface i, Just ignores) =
    ( n
    , APIInterface i
        { ifMethods = filterMethods (ifMethods i) ignores
        , ifSignals = filter ((`S.notMember` ignores) . sigName) (ifSignals i)
        , ifAllocationInfo = let ai = ifAllocationInfo i
                             in  case M.lookup n (allocInfo ovs) of
                                     Just info -> filterAllocInfo ai info
                                     Nothing   -> ai
        }
