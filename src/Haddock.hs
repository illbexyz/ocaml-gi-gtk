...
import           Config                         ( modName
                                                , overrides
                                                )
import           CtoHaskellMap                  ( Hyperlink(..) )
import           GtkDoc                         ( GtkDoc(..)
                                                , Token(..)
                                                , CRef(..)
                                                , Language(..)
                                                , Link(..)
...
formatUnknownCRef _ (LocalSignalRef s) =
  let sn = signalHaskellName s in "[" <> sn <> "](#signal:" <> sn <> ")"
...
formatHyperlink (ModuleLinkWithAnchor mLabel m a) = case mLabel of
  Nothing    -> "\"" <> m <> "#" <> a <> "\""
...
getDocBase = do
  ...
  return $ case M.lookup mod docsMap of
    Just url -> url
    Nothing  -> "http://developer.gnome.org/" <> T.toLower mod <> "/stable"
...
deprecatedPragma name (Just info) = do
  c2h     <- getC2HMap
  docBase <- getDocBase
  commentLine $ "DEPRECATED " <> name <> " " <> (T.pack . show)
    (note <> reason c2h docBase)
 where
  reason c2h docBase = case deprecationMessage info of
    Nothing  -> []
    Just msg -> map (formatHaddock c2h docBase . parseGtkDoc) (T.lines msg)
...
writeArgDocumentation arg = case rawDocText (argDoc arg) of
  Nothing  -> return ()
  Just raw -> do
    c2h     <- getC2HMap
    docBase <- getDocBase
    let haddock = "/@" <> lowerSymbol (argCName arg) <> "@/: " <> formatHaddock
          c2h
          docBase
          (parseGtkDoc raw)
...
writeReturnDocumentation callable skip = do
  ...
  let returnValInfo = if skip
        then []
          ...
          Nothing -> []
          Just raw ->
            ["__Returns:__ " <> formatHaddock c2h docBase (parseGtkDoc raw)]
  ...
  unless (T.null fullInfo) $ writeHaddock DocAfterSymbol fullInfo
