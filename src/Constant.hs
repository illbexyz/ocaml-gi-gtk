...
writePattern name (SimpleSynonym value t) =
...
genConstant (Name _ name) c = group $ do
  ...
  handleCGExc
    (\e ->
      line $ "(* Could not generate constant: " <> describeCGError e <> " *)"
    )
    (do
...
assignValue name t@(TInterface _) value = do
  ...
  case api of
    ...
    Just (APIFlags _) -> do
      ...
      writePattern
        name
        (ExplicitSynonym "gflagsToWord" "wordToGFlags" wordValue ht)
    _ ->
      notImplementedError
        $  "Don't know how to treat constants of type "
        <> tshow t
assignValue _ t _ =
  notImplementedError $ "Don't know how to treat constants of type " <> tshow t
...
showBasicType TBoolean b =
  notImplementedError $ "Could not parse boolean \"" <> b <> "\""
