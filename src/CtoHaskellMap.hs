...
cToHaskellMap apis = M.union (M.fromList builtins)
 ...
 where
  ...
  builtins =
    [ (TypeRef "gboolean" , TypeIdentifier "P.Bool")
...
constRefs n c =
  [ (ConstantRef (constantCType c), fullyQualifiedValue n (APIConst c) $ name n)
  , (TypeRef (constantCType c)    , fullyQualifiedValue n (APIConst c) $ name n)
  ]

-- | Extract the C name of a function.
funcRefs :: Name -> Function -> [(CRef, Hyperlink)]
funcRefs n f =
  [ ( FunctionRef (fnSymbol f)
...
enumRefs api n e =
  (TypeRef (enumCType e), fullyQualifiedType n api $ upperName n)
    : map memberToRef (enumMembers e)
 where
  memberToRef :: EnumerationMember -> (CRef, Hyperlink)
  memberToRef em =
    ( ConstantRef (enumMemberCId em)
    , fullyQualifiedValue n api $ upperName $ n
      { name = name n <> "_" <> enumMemberName em
      }
    )
  ...
  signalRef (Signal { sigName = sn }) =
    let mod        = dotModulePath (location n api)
        ...
        ownerCName = case maybeCName of
          Just cname -> cname
          Nothing    -> let Name ns owner = n in ucFirst ns <> owner
...
maybeCType n api (Just ctype) =
  [(TypeRef ctype, fullyQualifiedType n api (upperName n))]
...
ifaceRefs n i =
  maybeCType n (APIInterface i) (ifCType i)
...
objectRefs n o =
  maybeCType n (APIObject o) (objCType o)
